...
export default function MonteCarloTab({ ctx }) {
  ...
  return (
    <div style={{ padding: '24px 28px', background: '#F5F3EF', minHeight: '100%' }}>
      <h2 style={{ fontSize: 22, color: '#1A1A1A', marginBottom: 4, fontWeight: 600 }}>Stress Test</h2>
      ...
      {/* Scenario selector */}
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E4DC', borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          ['Median Final', fmtC(pctData.length > 0 ? pctData[pctData.length - 1].p50 : 0), '#2563eb'],
          ['90th Pct',     fmtC(pctData.length > 0 ? pctData[pctData.length - 1].p90 : 0), '#7c3aed'],
          ['10th Pct',     fmtC(pctData.length > 0 ? pctData[pctData.length - 1].p10 : 0), '#dc2626'],
        ].map(function(item) {
          return (
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E4DC', borderRadius: 12, padding: '20px', marginBottom: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={pctData} margin={{ top: 10, right: 20, left: 20, bottom: 0 }}>
            <defs>
              {[['gp90','#7c3aed'],['gp75','#2563eb'],['gp50','#059669'],['gp25','#d97706'],['gp10','#dc2626']].map(function(g) {
                return (
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E8E4DC', borderRadius: 12, padding: '20px', marginBottom: 20 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={bucketPctData} margin={{ top: 10, right: 20, left: 20, bottom: 0 }}>
              <defs>
                {[['gb1','#34d399'],['gb2','#60a5fa'],['gb3','#a78bfa']].map(function(g) {
                  return (
                    <linearGradient key={g[0]} id={g[0]} x1="0" y1="0" x2="0" y2="1">
              ...
              <Tooltip content={<TTip/>}/>
              <Legend wrapperStyle={{ fontSize: 11, color: '#6B7280' }}/>
              <Area type="monotone" dataKey="b1" stroke="#34d399" fill="url(#gb1)" strokeWidth={1.5} name="Bucket 1 (Cash)"   fillOpacity={1}/>
              <Area type="monotone" dataKey="b2" stroke="#60a5fa" fill="url(#gb2)" strokeWidth={1.5} name="Bucket 2 (Income)" fillOpacity={1}/>
              <Area type="monotone" dataKey="b3" stroke="#a78bfa" fill="url(#gb3)" strokeWidth={2}   name="Bucket 3 (Growth)" fillOpacity={1}/>
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
